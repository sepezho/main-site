		...
		return (
			<div key={instPost.id} className={s.Slide}>
				...
				<div>
					...
					<div>
						<img src={Heart} alt="" />
						<span>{instPost.edge_liked_by.count}</span>
					</div>
					<div>
						<img src={Comments} alt="" />
						<span>{instPost.edge_media_to_comment.count}</span>
					</div>
					<div className={s.SliderNote}>
						<img src={File} alt="" />
						<span>
							{instPost.edge_media_to_caption.edges[0]?.node.text
								? instPost.edge_media_to_caption.edges[0]?.node.text.length >=
								  70
									? instPost.edge_media_to_caption.edges[0]?.node.text
											.split(`\n`)[0]
											.slice(0, 70) + "..."
		...
		if (props.instData.edge_owner_to_timeline_media?.edges[0]) {
			return (
				<Slider className={s.Slider} {...settings}>
					{props.instData.edge_owner_to_timeline_media.edges
			...
			<div className={s.SliderContainer}>
				{useResponsive("(min-width: 1024px)", true) ? (
					<Tilt
						children={
							<FilledSlider
								className={s.SliderContainerItem}
								instData={props.instData}
							/>
						}
					/>
				) : (
